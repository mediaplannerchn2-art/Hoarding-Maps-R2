(() => {
  ...
  const mapLibreRender = $('mapLibreRender');
  ...
  function verifyProjectionAccuracy(rows, pixels, map) {
    const failures = [];
    rows.forEach((row, i) => {
      const lngLat = map.unproject(pixels[i]);
      const lat2 = lngLat.lat;
      const lon2 = lngLat.lng;
  ...
  function waitForMapLibreIdle(map, timeoutMs = 30000) {
    return new Promise((resolve, reject) => {
      ...
      const onError = (event) => {
        cleanup();
        reject(new Error(event?.error?.message || 'MapLibre could not load the map tiles.'));
      };
      const timer = setTimeout(() => {
        cleanup();
        reject(new Error('MapLibre map loading timed out.'));
      }, timeoutMs);
      map.on('idle', onIdle);
  ...
  async function drawMapLibreBase(context, centerNorm, zoom, width, height) {
    if (window.mapLibreReady) {
      try {
        await window.mapLibreReady;
      } catch (error) {
        throw new Error(`MapLibre GL JS could not load: ${error?.message || String(error)}`);
      }
    }
    if (typeof maplibregl === 'undefined') {
      throw new Error('MapLibre GL JS did not load. Check your internet connection and refresh the page.');
    }

    if (state.mapRenderer) {
      state.mapRenderer.remove();
      state.mapRenderer = null;
    }
    mapLibreRender.replaceChildren();
    mapLibreRender.style.width = `${width}px`;
    mapLibreRender.style.height = `${height}px`;

    const [centerLat, centerLon] = normToLatlon(centerNorm[0], centerNorm[1]);
    const map = new maplibregl.Map({
      container: mapLibreRender,
    ...
    state.mapRenderer = map;
    map.resize();
    renderInfo.textContent = 'Loading map with MapLibre GL JS…';
  ...
  async function renderMap(rows, width, height, highlightLabels) {
    ...
    const map = await drawMapLibreBase(ctx, center, zoom, width, height);

    const markerRadius = Math.max(13, Math.round(height * 0.0165));
    const markerPixels = rows.map((row) => {
      const point = map.project([row.Long, row.Lat]);
      return [point.x, point.y];
    });
    verifyProjectionAccuracy(rows, markerPixels, map);
  ...
  async function generate() {
    ...
    try {
      ...
      console.error(error);
      showStatus(
        `${error.message || String(error)}\n\nIf the map fails, confirm that MapLibre GL JS and OpenStreetMap tile requests are not being blocked by your network.`,
        'error'
      );
